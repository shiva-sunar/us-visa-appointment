...
(async () => {
  ...
  async function runLogic() {
    ...
    const browser = await puppeteer.launch({
      args: ["--proxy-server=127.0.0.1:8080", "--start-maximized"], //added by Shiva
      ignoreHTTPSErrors: true, //Added by Shiva
      headless: false
    });
    ...
    // page.setIgnoreHTTPSErrors(true); // Added by Shiva
    //#endregion
    ...
    // We are logged in now. Check available dates from the API
    {
      ...
      if (availableDates.length <= 0) {
        ...
        await new Promise(resolve => setTimeout(resolve, 100000)); // Wait for 10 seconds Added by Shiva
        await browser.close();
        return false;
      }
  ...
  while (true) {
    try {
      ...
      notify("Hello Shiva, Some error occurred during the process!!!");
    }
